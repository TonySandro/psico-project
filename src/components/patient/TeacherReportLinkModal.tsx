...
import { useTeacherReportTemplates } from '@/hooks/useTeacherReportV2';
...
export default function TeacherReportLinkModal({ open, onClose, patientId }: TeacherReportLinkModalProps) {
  const { mutate: generateLink, isPending, error, data } = useGenerateTeacherReportLink();
  const { data: templates } = useTeacherReportTemplates();
  ...
  const fullUrl = data?.token ? `${window.location.origin}/teacher-report/responder/${data.token}` : '';
  ...
  const handleEmail = () => {
    const subject = encodeURIComponent('Link de Relatório do Professor');
  ...
  return (
    <Dialog open={open} onClose={isPending ? undefined : onClose} maxWidth="sm" fullWidth>
      <DialogTitle>Link de Relatório do Professor</DialogTitle>
      <DialogContent>
        {isPending && (
        ...
        {data && !isPending && (
          <Stack spacing={3} sx={{ mt: 1 }}>
            <Typography variant="body2" color="text.secondary">
              Este link pode ser enviado ao professor para responder o relatório. Ele não requer login para ser acessado.
            </Typography>
            ...
            <Box>
              ...
              <Stack direction="row" alignItems="center" spacing={1}>
                ...
                <Typography variant="body2" fontWeight={600}>
                  {data.status === 'ANSWERED' ? 'Respondido' : 'Pendente'}
                </Typography>
              </Stack>
            </Box>
